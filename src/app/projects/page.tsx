...
const VALID_TYPES = ["SOFTWARE", "ROBOTICS", "HARDWARE", "RESEARCH"] as const;
...
export default async function ProjectsPage({
  ...
  const activeType = VALID_TYPES.find((t) => t === searchParams.type) ?? null;
  const all = await getProjects();
  const projects = activeType ? all.filter((p) => p.type === activeType) : all;
      ...
      <main className="min-h-screen pt-24 pb-16">
        <div className="section-container">
          ...
          <p className="text-slate-400 mt-4 mb-2">
            Software, robotics, hardware, and research.
          </p>
          <ProjectFilterTabs activeType={activeType} />
